...
const NovaPesquisa = props => {
  ...
  const addImg = () => {
    Alert.alert(
      ...
      [
        {
          text: 'Câmera',
          onPress: () => launchCamera({ mediaType: 'photo', cameraType: 'back', quality: 1 }, handleImageSelection),
        },
        {
          text: 'Galeria',
          onPress: () => launchImageLibrary({ mediaType: 'photo', quality: 1 }, handleImageSelection),
        },
  ...
  const handleImageSelection = (response) => {
    if (response.didCancel) {
      ...
      setUrlImg(response.assets[0].uri);
